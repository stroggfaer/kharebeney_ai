...
void menu_init(SSD1306_t* oled) {
    ...
    MenuState initState = { .selectedIndex = 0, .currentScreen = 'E' };
...
void menu_draw(SSD1306_t* oled, const MenuState* state) {
    ...
    for (int i = 0; i < topCount; i++) {
        char tmp[8];
        if (state->selectedIndex == i) snprintf(tmp, sizeof(tmp), "[%s] ", topMenu[i]);
        else snprintf(tmp, sizeof(tmp), " %s  ", topMenu[i]);
        strcat(topBuf, tmp);
    }

    for (int i = 0; i < bottomCount; i++) {
        char tmp[8];
        if (state->selectedIndex == i + topCount) snprintf(tmp, sizeof(tmp), "[%s] ", bottomMenu[i]);
        else snprintf(tmp, sizeof(tmp), " %s  ", bottomMenu[i]);
    ...
    char topRight[32];
    snprintf(topRight, sizeof(topRight), "%s %s", timeBuf, battBuf);
    ssd1306_display_text(oled, 1, topRight, strlen(topRight), false);
...
void menu_handle_input(SSD1306_t* oled, MenuState* state, int btnA, int btnB, int btnC) {
    ...
    if (btnB == 0) {
        menu_draw(oled, state); // подтверждение выбора (заглушка)
    }
    if (btnC == 0) {
        state->selectedIndex = 0;
        state->currentScreen = 'E';
        menu_draw(oled, state);
    }
}